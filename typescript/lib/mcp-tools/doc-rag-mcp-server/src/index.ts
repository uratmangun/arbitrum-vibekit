...
async function handleIndexDocumentation(params: IndexDocumentationParams) {
  try {
    ...
    const { pages, errors } = await crawler.crawl((progress) => {
      console.error(`Progress: ${progress.scrapedPages}/${progress.totalPages} - Currently scraping: ${progress.currentUrl}`);
    });
    ...
    let embeddingStats = { chunksCreated: 0, embeddingsGenerated: 0, estimatedCost: 0 };
        ...
        for (const page of pages) {
          ...
          chunks.forEach(chunk => allChunks.push({ chunk, pageUrl: page.url }));
            ...
            // Generate embeddings
            const embeddings = await embeddingsGenerator.generateEmbeddings(allChunks.map(item => item.chunk));
            embeddingStats.embeddingsGenerated = embeddings.size;
            
            // Store in vector store
            vectorStore.addBatch(allChunks.map(item => item.chunk), embeddings);
                } catch (error) {
            // Continue even if embeddings fail - don't log to avoid protocol contamination
          }
        }
...
async function handleQueryDocumentation(params: QueryDocumentationParams) {
  try {
    ...
    const response = {
      ...
      ragInstruction: "Use ONLY the documentation chunks provided above to answer questions. Always cite the source URL when referencing information."
    };
...
async function main() {
  ...
  try {
    ...
    console.error(`Environment check - OPENAI_API_KEY: ${process.env.OPENAI_API_KEY ? 'SET' : 'NOT SET'}`);
